...
for im in "${IMAGES[@]}"; do
  ...
  for KER in "${KERNELS[@]}"; do
    ...
    if [ "$im" = "im05" ] && { [ "$kbase" = "kernel49x49_random" ] || [ "$kbase" = "kernel99x99_random" ]; }; then
      HRT="06:00:00"
    fi

    qsub -N "ser_${im}_${kbase}" -l h_rt="$HRT" \
      ~/hpc_conv/scripts/run_serial_conv.sh \
      ~/hpc_conv/bin/serial_convolution \
      "$IMG" "$KER" "$PART"
  done
done
